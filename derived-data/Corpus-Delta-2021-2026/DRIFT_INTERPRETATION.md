...
(per-text WordSem / Treebank / IsMantra token counts + only-2026 flag, generated by
[`delta_annotation_layers.py`](https://github.com/gasyoun/VisualDCS/blob/main/derived-data/Corpus-Delta-2021-2026/delta_annotation_layers.py); see §6) ·
...
_(One story **at token level** — §3b adds a second, orthogonal axis: annotation depth, where the picture differs.)_
...
## 3b. The delta has a second, orthogonal axis: annotation depth (not just tokens)
...
| Layer | What it is | Texts (of 270) | Vedic-selective? |
|---|---|---:|---|
| **`WordSem`** | lexical semantic-concept IDs → [Sanskrit WordNet](http://sanskritwordnet.unipv.it/) | **219** | **No — corpus-wide** |
| **Vedic Treebank** (`HEAD`/`DEPREL`) | dependency syntax | **74** (~61 real + trace) | **Yes** |
| **`IsMantra`** | token is in a mantra (Bloomfield's Vedic Concordance) | **44** | **Yes** (by definition) |

So "the DCS added *semantic* markup to selected Vedic texts" inverts the truth twice:
semantic (`WordSem`) markup is the **broad** layer (only 51 texts lack it), and the
genuinely Vedic-selective layers are **syntax** (Vedic Treebank) and **`IsMantra`**.

**The sharp finding — the two axes are anti-correlated at the delta.** Of the 30 only-2026
texts (the entire "went Vedic" wave of §1), **29 arrived with *zero* `WordSem`** — the sole
exception is Atharvaveda (Paippalāda), 6,403 semantic tokens. The Vedic wave added raw
tokens **without** the semantic layer. Hence the delta is really *two* stories, not one:
**text growth** (Vedic, semantically unannotated) and **annotation depth** (the semantic
layer is an old-corpus asset; the syntax/mantra layers are the Vedic-specific enrichment).
Any downstream use must not assume the new Vedic tokens are WordNet-linked — they are not.
