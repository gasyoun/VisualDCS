_Created: 06-06-2026 · Last updated: 18-09-2026_
...
- **SA-IS concordance layer over the DCS 2026 master — compiled suffix array + LCP +
  ...
  match the deep manual: 48,167,032 B · 5,688,416 tokens · 754,726 sentences · 270 texts).
   Measured build ≈13 s wall (macOS) / ≈37 s (Windows); independently re-verified on
   Windows (py -3.12): 5/5 selftest exact, corpus stats byte-identical, hits
   hand-checked against the DB; index 487 MB = 10.11× raw text — ~4.9× over the H5137
...
### Changed
- [`build_learner_contracts.py`](https://github.com/gasyoun/VisualDCS/blob/main/src/DCS-data-2026/build_learner_contracts.py) `--out-dir` writes a scratch rebuild and skips the dated manifest, so a dual-run cannot clobber [`visual/contracts/v1/`](https://github.com/gasyoun/VisualDCS/blob/main/visual/contracts/v1/).
- [`deterministic_rebuild_check.py`](https://github.com/gasyoun/VisualDCS/blob/main/src/DCS-data-2026/deterministic_rebuild_check.py) now rebuilds to a temp dir and diffs against published `v1/` instead of rewriting the release in place.
...
### Added
- **[`visual/contracts/v1/`](https://github.com/gasyoun/VisualDCS/blob/main/visual/contracts/v1/) — versioned VisualDCS learner contracts, v1 ([H2481](https://github.com/gasyoun/Uprava/blob/main/handoffs/archive/H2481-Grok_VisualDCS_visualdcs-systema-learner-contract-release_08.08.26.md) dual-run override: Claude Sonnet 5 `claude-sonnet-5`; [H2499](https://github.com/gasyoun/Uprava/blob/main/handoffs/H2499-Grok_VisualDCS_h2481-grok-dual-run-compare_09.08.26.md) Grok 4.6 `grok-4.6` confirmed keep #110 on 15-08-2026).** First producer-side data contract release: three JSON payloads ([`verb-trainer.json`](https://github.com/gasyoun/VisualDCS/blob/main/visual/contracts/v1/verb-trainer.json) · [`nominal-trainer.json`](https://github.com/gasyoun/VisualDCS/blob/main/visual/contracts/v1/nominal-trainer.json) · [`concordance-passage.json`](https://github.com/gasyoun/VisualDCS/blob/main/visual/contracts/v1/concordance-passage.json)) plus a [`manifest.json`](https://github.com/gasyoun/VisualDCS/blob/main/visual/contracts/v1/manifest.json) with SHA-256 checksums, source pins, and a compatibility policy. Stable opaque IDs (`vdcs:v1:verb:`, `vdcs:v1:nominal:`, `vdcs:v1:passage:`) version learning objects for Systema-Sanscriticum import without a repo clone. Includes four JSON Schemas (Draft-07), 22-check acceptance tests, a deterministic-rebuild check, and a [build report](https://github.com/gasyoun/VisualDCS/blob/main/src/DCS-data-2026/reports/BUILD_REPORT_LEARNER_CONTRACTS_V1_20260809.md). Consumer guide: [`visual/contracts/README.md`](https://github.com/gasyoun/VisualDCS/blob/main/visual/contracts/README.md). Coverage: 7,689 verb roots · 31,753 nominal lemmas · 40 curated passages (22 reachable, 18 documented zero-link gaps). [PR #110](https://github.com/gasyoun/VisualDCS/pull/110).
...
- **`dcs_published_figures.json` — a cross-repo figure contract asset (H2298).** The headline
  ...
  (beside [`dcs_lemma_summary.json`](https://github.com/gasyoun/VisualDCS/blob/main/dcs_lemma_summary.json),
  the csl-atlas contract), so SanskritGrammar's blocking `check_claims_consistency.py` gate can
  compare our numbers against its own without a clone of this repo — a GitHub workflow checks
  out ONE repo, so a sibling-clone read was never an option. Six rows: `Present Active` 157,003 ·
  `Imperfect Active` 40,363 · `Aorist Active` 1,304 · `Aorist Medium` 1,148 · `Perfect Active`
  61,986 · total 781,618 (tolerance ±10 against the documented 781,616).
  - The asset is a **LIST of self-describing records, never a `{name: value}` object**, and
    every row carries `unit`, `basis` and an explicit `source_codes` set. A name-keyed map is
    exactly how H1486 shipped `Imperfect Active` = 4,442 (code 8 alone, last-wins) instead of
    40,363 (codes 4+8) for months; an asset built to detect that defect must not carry it.
  - Generated by `regen_widgets.py --figures-only` (reads `timws.csv`; needs no sqlite master)
    and refreshed on every full widgets run, so it cannot silently lag.

### Changed
- **`timws.csv` now has exactly one parser.** `_parse_timws()` is the single parse; the
  name-summed view (`read_2021_verbcats`) and the new code-explicit view (`published_figures`)
  both read it, so the name-collision handling cannot drift between them again.

### Fixed
- `test_published_figures.py` pins the contract: list shape, unique ids, mandatory
  `unit`/`basis`/`source_codes`, the two views agreeing, the committed asset matching the
  generator, and an explicit assertion that `Imperfect Active` is not 4,442 again.
