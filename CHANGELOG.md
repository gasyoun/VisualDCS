...
- **Contract assets get a live regen trigger (Uprava H5423, E016 wave 5)** — new
  ...
  `dcs_lemma_summary.json` (`gen_dcs_lemma_summary.py`) whenever `timws.csv`, `_8.csv` or a
  generator changes: it refuses the drift on a PR and commits the result back on `main`.
  Before, both were regenerated by hand and CI only validated the committed copy.
  [`check_generated_figures.py`](https://github.com/gasyoun/VisualDCS/blob/main/check_generated_figures.py)
  compares parsed JSON without the `generatedAt` run date (the lemma summary is one line,
  so a line mask would hide everything). Both assets are current today (verified by a
  regen: figures byte-identical, lemma summary date-only). Opus 5.5 (`claude-opus-5-5`).
